...
% StartThresh = 2; % Maximal allowable start level above baseline
% info.GraphLimits = [-0.25 8]; % Min/max used in graphing-
% AreaThresh = 90;
dendro = 0;
colors = setcolors;
...
droprows =  [droprows, prctile(nfkb(:,1:8),18.75,2) > StartThresh];
droprows =  [droprows, nanmedian(measure.MeanIntensityNuc(:,1:31),2) > nuc_thresh];
droprows =  [droprows, nanmedian(measure.Area,2) < AreaThresh];
...
if verbose_flag
    ...
    h = suptitle(['x = NFkB activation @ start. Threshold = ',num2str(StartThresh)]);
    set(h,'FontSize',14)
    ...
    h = suptitle(['x = Median area. Threshold = ',num2str(AreaThresh)]);
    set(h,'FontSize',14)
     end
